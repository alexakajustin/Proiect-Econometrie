...
rm(list = ls()) 
directory <- "C:/Users/40726/Desktop/Econometrie 2023/PROIECT_ECONOMETRIE_1084_Dumitriu_Ana_Maria_Dumitrescu_Teodora_Florea_Mihaela_Gabriela/"
...
# pentru un interval de incredere de de 10% putem estima urmatoarele valori: 1  67076.92  65042.81  69111.03
...
model_0 <- lm(PriceEuro ~  DummySeats5+ TopSpeed+ Range, price1)
...
ols_plot_resid_fit(model_0)    #grafic din libratrie
...
# pentru un interval de incredere de de 10% putem estima urmatoarele valori (masina 2): 2  97244.11  89627.94 104860.28
...
hprice1 %<>% mutate(HB_FWD = DummyBodyStyle * DummyFWD, 
                    notHB_FWD = notBodyStyle * DummyFWD, HB_notFWD = DummyBodyStyle * notFWD)

# Afisam ista variabilelor create pentru primele 10 observatii
hprice1 %>% 
  select(DummyBodyStyle, notBodyStyle, DummyFWD, notFWD, HB_FWD, notHB_FWD, HB_notFWD) %>%
  head(10)

# Regresia cu cele 4 categorii
...
model_interactiune_dummy <- lm(PriceEuro ~ DummyBodyStyle + DummyFWD + DummyBodyStyleXDummyFWD, hprice1)
...
# Model restrictionat: PriceEuro= alpha0 + alpha1*TopSpeed + alpha2*Range + alpha3*Accel + e
...
model_10 <- lm(PriceEuro ~ TopSpeed + Range + Accel + TopSpeedXHB + 
...
# Daca F-stat =0.95 < F-critical(2.47) => nu respingem ipoteza nula H0,
...
model_log <- lm(lPriceEuro ~  DummySeats5+ lTopSpeed+ lRange, hprice1)    #Bonitatea = 85.99%, coef nu sunt semmnificativi
summary(model_log)
# completam datele variabilelor independente pentru o masina 
prognoza <- data.frame(DummySeats5 = c(1), lTopSpeed = c(200), lRange = c(300))
y_pred_scenariu <- predict(model_log, newdata = prognoza)
y_pred_scenariu           # valoarea este logaritmata
...
# Interpretare: Daca masina are fix 5 locuri, pretul ei este cu 38 69 euro mai mic
...
predict(best_model, s = best_lambda, newx = new)  # Pretul previzionat este 70 519 euro
...
# Progoza pentru masina cu 5 locuri, viteza maxima 210, autonomie 400 (3 coloane completate)
new <- matrix(c(1, 210, 400), nrow=1, ncol=3) 
predict(best_model, s = best_lambda, newx = new)   # Pretul estimat este 72 773.92 euro
...
library(Boruta)
set.seed(111)
boruta.bank_train <- Boruta(PriceEuro~., data = hprice1, doTrace = 2)
...
model_Boruta1 <- lm(lPriceEuro ~  Brand + Accel+ TopSpeed + Range+ Efficiency + PowerTrain + Segment + Seats, hprice1)    #r=98.82%
summary(model_Boruta1)     

# Modelul Boruta in functie de variabilele numerice preluate din csv
model_Boruta2 <- lm(PriceEuro ~ Accel + TopSpeed +  Efficiency + Range, hprice1)
summary(model_Boruta2)   # Bonitatea modelului = 90.76%. Modelul este semnificativ la 99% (p-value: < 2.2e-16)
...
# Forma liniara: PriceEuro = -102000 + 8480 * Accel + 642.7* TopSpeed + 182.5 * Efficiency - 0.7152 * Range
...
model_Boruta3 <- lm(PriceEuro ~ Accel + TopSpeed+  Efficiency +Range + DummySegmentB +DummySeats5, hprice1)
summary(model_Boruta3)    # Bonitatea modelului = 91.17%. Modelul este semnificativ la 99% (p-value: < 2.2e-16)

# Forma liniara: PriceEuro = -105700 + 8786* Accel + 630.7* TopSpeed + 207.1 * Efficiency - 6.17 * Range + 
